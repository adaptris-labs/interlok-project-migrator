...
public class ProjectConverterTest {
  ...
  public void convertMap() throws Exception{
    ...
    Map<String, String> results = new ProjectConverter().convert("<foo><foo1>Foo Test 1</foo1><services><another1><unique-id>value</unique-id><test1>Foo Test 2.0</test1><test1>${data}</test1></another1></services><foo3>Foo Test 3</foo3><foo4>Foo Test 4</foo4></foo>", variables);
  ...
  public void convertMapJMSExample() throws Exception {
    ...
    Map<String, String> results  = projectConverter.convert(projectConverter.readFile(adpXml.getAbsolutePath(), StandardCharsets.UTF_8), projectConverter.loadProperties(vars.getAbsolutePath()));
