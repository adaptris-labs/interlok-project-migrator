...
public class ProjectMigratorTest {
  ...
  public void convertMapJMSExample() throws Exception {
    ...
    Map<String, String> results  = projectMigrator.convert(projectMigrator.readFile(adpXml.getAbsolutePath()), projectMigrator.loadProperties(vars.getAbsolutePath()));
  ...
  private void assertResults(Map<String, String > results ) {
    ...
    assertEquals("Flat ${HOUSE_NUMBER}", results.get("/adapter/channel-list/channel[unique-id=\"sleepy-murdock\"]/workflow-list/standard-workflow[unique-id=\"pedantic-lovelace\"]/service-collection/services/add-metadata-service[unique-id=\"kickass-lamarr\"]/metadata-element[1]/value"));
  }
}